        ...
        <div class="container mx-auto py-8 px-4">
            <div class="flex flex-col md:flex-row bg-white shadow-lg">
                ...
                <div class="md:flex-none w-full md:w-1/3 justify-center p-6 rounded-lg mb-4 md:mb-0 md:mr-4">
                    <div class="mb-6">
                        <img
                                ...
                                src="{{ $share->resume->person->getAvatar() }}"
                                alt="{{ $share->resume->owner->full_name }}"
                        >
                    </div>

                    <div class="flex justify-center">
                        <div>
                            <h1 class="text-xl font-semibold text-center">{{ $share->resume->owner->full_name }}</h1>
                            <p class="text-gray-500 text-center">{{ $share->resume->person->position }}</p>
                        </div>
                    </div>
                    <hr class="my-4">
                    <ul class="flex flex-col space-y-2 list-disc list-inside text-gray-700">
                        @if ($share->resume->person->location)
                        ...
                        @if ($share->resume->person->location)
                            <li class="flex items-center">
                                ...
                                {{ $share->resume->person->location }}
                            </li>
                        @endif

                        @if ($share->resume->person->linkedin)
                            <li class="flex items-center">
                                ...
                                {{ $share->resume->person->linkedin }}
                            </li>
                        @endif
                            
                        @if ($share->resume->person->website)
                            <li class="flex items-center">
                                ...
                                <a href="{{ $share->resume->person->website }}" class="text-blue-500 hover:underline">{{ $share->resume->person->website }}</a>
                            </li>
                        @endif
                    </ul>

                    @if ($share->resume->person->phone)
                        <x-link
                                class="w-full my-6 justify-center text-center" :href="'tel:' . $share->resume->person->phone"
                        >
                    ...
                    @else
                        <x-link
                                class="w-full my-6 justify-center text-center" :href="'mailto:' . $share->resume->owner->email"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4 mr-2">
                                <path d="M1.5 8.67v8.58a3 3 0 0 0 3 3h15a3 3 0 0 0 3-3V8.67l-8.928 5.493a3 3 0 0 1-3.144 0L1.5 8.67Z"/>
                                <path d="M22.5 6.908V6.75a3 3 0 0 0-3-3h-15a3 3 0 0 0-3 3v.158l9.714 5.978a1.5 1.5 0 0 0 1.572 0L22.5 6.908Z"/>
                ...
                <div class="w-full p-6 rounded-lg bg-white shadow-lg">
                    ...
                    <hr class="my-4">
                    @unless(empty($share->resume->person->bio))
                        <p class="text-gray-700">{!! str($share->resume->person->bio)->markdown() !!}</p>
                    @else
                    ...
                    <x-skills.list-content>
                        @forelse ($share->resume->skills as $skill)
                    ...
                    <x-experiences.content>
                        @forelse ($share->resume->experiences as $experience)
                    ...
                    <x-educations.content>
                        @forelse ($share->resume->educations as $education)
                    ...
                    <hr class="my-4">
                    <x-languages.content>
                        @forelse ($share->resume->languages as $language)
                    ...
                    <x-certificates.content>
                        @forelse ($share->resume->certificates as $certificate)
